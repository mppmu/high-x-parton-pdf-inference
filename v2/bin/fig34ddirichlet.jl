...
# Joint posterior
plot!(prior_samples, (:(K_d), :(θ[2])),
    subplot=3,
    xlabel=L"K_d", ylabel=L"\Delta_d",
...
plot!(samples_data, (:(K_d), :(θ[2])), 
    ...
    xlims=xlims_K_d, ylims=xlims_D_d
    , xtickfontsize=14,ytickfontsize=14,yguidefontsize=16,xguidefontsize=16, legendfontsize=14
    , right_margin=-2mm
    , left_margin=6mm
    , top_margin=0mm
...
plot!(samples_data, :K_d,
    subplot=1, 
    legend=false, xlabel="", ylabel=L"P(K_d)",
...
plot!(samples_data, :(θ[2]),
    subplot=4,
    legend=false, ylabel="", xlabel=L"P(\Delta_d)", 
    ylims=xlims_D_u, xlims=(0, 25), 
